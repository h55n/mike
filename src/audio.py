...
class AudioCapture:
    ...
    def stop_capture(self) -> io.BytesIO | None:
        ...
        if avg_rms < MIN_AVG_RMS:
            logger.info(
                f"Rejecting capture — avg RMS {avg_rms:.0f} below threshold {MIN_AVG_RMS} (silence/ambient)"
            )
            return None
        ...
        if self._voice_frames < MIN_VOICE_FRAMES:
            logger.info(
                f"Rejecting capture — only {self._voice_frames} voice frames (likely noise)"
            )
            return None
        ...
        buf.seek(0)
        logger.debug(
            f"Audio captured: {len(audio_data)/SAMPLE_RATE:.2f}s, {self._voice_frames} voice frames"
        )
        return buf
